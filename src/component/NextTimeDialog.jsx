...
import { Form, Dialog, Button } from 'element-react/next'
...
function NextTimeDialog(props) {
    const { user, boss, onSetNextTime, onSetBossNextTime, onCancel } = props
    ...
    const submitHandler = e => {
        ...
        if (user) {
            onSetBossNextTime(id || user.uid, boss.key, minutes)
        } else {
            onSetNextTime(boss.key, minutes)
        }
        onCancel()
    }
    return (
...
const mapDispatch2Props = dispatch => ({
    onSetNextTime: (bossKey, afterMinutes) => dispatch(setNextTime({key:bossKey, afterMinutes})),
    onSetBossNextTime: (userId, bossKey, afterMinutes) => dispatch(setBossNextTime(userId, bossKey, afterMinutes)),
